...
export default function Conversor2() {
  ...
  const handleConvert = () => {
    ...
    if (eurValue) {
      setCurrencyValue(
        (parseFloat(eurValue) * selectedProduct.price).toFixed(2)
      );
    } else if (currencyValue) {
      setEurValue(
        (parseFloat(currencyValue) / selectedProduct.price).toFixed(2)
      );
    }
  };
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-around" }}>
        <div>
          ...
          <p>
            Seleccionado: {selectedProduct ? selectedProduct.name : "Ninguno"}
          </p>
        </div>
      </div>
      <div>
